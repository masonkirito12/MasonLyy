...
if(!empty($_POST['cancel'])){
    $qry="UPDATE reserve set status = 'delete' WHERE id='".$_POST['cancel']."'";
                ...
                <div class="card-content table-responsive">
                    ...
                    <tbody id="myTables">
                        <?php 
                            ...
                            // Borrow Book
                                $borrowbook = "SELECT * FROM book_detail WHERE book_id = '".$row['id']."'";
                                $resultborrowbook = mysqli_query($conn,$borrowbook);
                                $rowborrowbook = mysqli_fetch_assoc($resultborrowbook);
                            //reserve
                                $selectreserve="SELECT MIN(id) AS newid FROM reserve WHERE book_id='".$row['id']."' and status='success'";
                            ...
                            //checking Borrow
                                $checkborrow = "SELECT * FROM borrow WHERE status = 'return' AND member_id = '".$_SESSION['id']."'";
                                $resultcheckborrow = mysqli_query($conn,$checkborrow);
                                $borrowcheck = mysqli_fetch_assoc($resultcheckborrow);
                            //checking Pending
                                $checkpending = "SELECT * FROM borrow WHERE status = 'pending' AND member_id = '".$_SESSION['id']."'";
                                $resultcheckpending = mysqli_query($conn,$checkpending);
                                $pendingcheck = mysqli_fetch_assoc($resultcheckpending);
                            //Checking reserve
                                $checkreserve = "SELECT * FROM reserve WHERE status = 'success' AND member_id = '".$_SESSION['id']."'";
                                $resultcheckreserve = mysqli_query($conn,$checkreserve);
                                $reservecheck=mysqli_num_rows($resultcheckreserve);
                            //Cancel reserve
                                $cancelreserve = "SELECT * FROM reserve WHERE member_id = '".$_SESSION['id']."'  AND status = 'success' AND book_id='$row[id]'";
                                $resultcancelreserve = mysqli_query($conn,$cancelreserve);
                                $reservecancel=mysqli_num_rows($resultcancelreserve);
                                $rowreservecancel=mysqli_fetch_array($resultcancelreserve);
                                if($reservecancel>0){
                                    $btn="<button type='submit' class='btn btn-danger' name='cancel' value='$rowreservecancel[0]'>Cancel</button>";
                                }else{
                                    $btn="";
                                }
                            ?>
                            <tr>
                                ...
                                if ($reservecheck<=2) {
                                    if ($borrowcheck==0 and $pendingcheck==0) {
                                        if($row_borrow['quantity'] <= 0 and $rowreserve['newid']!==""){ 
                                ?>
                                <a href="reserve.php?id=<? echo $rowborrowbook['book_id'];?>"><i class="material-icons">shopping</i></a>
                                <?
                                        }   
                                    }
                                }
